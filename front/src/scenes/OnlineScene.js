class OnlineScene extends Scene {
	...
	init() {
		...
		socket.on("challengeOpponent", (key) => {
			...
			alert(
				`Перший хто перейде за цим посиланням буде грати з вами:\n${location.href}`
			);
		});
	...
	statusUpdate() {
		...
		if (!this.status) {
			...
			statusDiv.textContent = "Пошук випадкового суперника";
		} else if (this.status === "play") {
			statusDiv.textContent = this.ownTurn ? "Ваш хід" : "Хід суперника";
		} else if (this.status === "winner") {
			statusDiv.textContent = "Ви перемогли";
		} else if (this.status === "loser") {
			statusDiv.textContent = "Ви програли";
